...
with lib;
{
  options.my.tools.podman.machine = {
  ...
  config = mkIf config.my.tools.podman.machine.enable {
    xdg = {
      enable = true;
      configFile = {
        "containers/podman/machine/qemu/${cfg.name}.json".text = builtins.toJSON {
          ...
          IgnitionFilePath = "${conf.user.home}/.config/containers/podman/machine/qemu/${cfg.name}.ign";
          ImageStream = "testing";
          ImagePath = "${conf.user.home}/.local/share/containers/podman/machine/qemu/${cfg.name}_fedora-coreos-36.20220421.dev.0-qemu.aarch64.qcow2";
          Memory = cfg.memory;
          DiskSize = cfg.diskSize;
          Name = "${cfg.name}";
          Port = 57056;
          QMPMonitor = {
            Address = "${config.xdg.dataHome}/containers/podman/machine/${cfg.name}/qmp.sock";
            Network = "unix";
            Timeout = 2000000000;
          };
          RemoteUsername = "core";
