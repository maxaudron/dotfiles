...
let
  ...
  contribSrc = fetchFromGitHub {
    ...
    repo = "opencv_contrib";
    tag = version;
    hash = "sha256-+z8vIGofdQPnjh5D7A8TQWXwTGRwTxSUBo1W/RdpIE4=";
  };
...
effectiveStdenv.mkDerivation {
  ...
  patches = [
    ...
    # Backport fix for opencv 5.0.0 dnn build with system protobuf:
    # "dnn: move generic protobuf I/O helpers out of src/caffe" (opencv#29425)
    (fetchpatch2 {
      name = "opencv-5.0.0-dnn-move-protobuf-io-out-of-src-caffe.patch";
      url = "https://github.com/opencv/opencv/pull/29425.patch?full_index=1";
      hash = "sha256-TwFk97rR000pLkJCv+4sCkENLRXJGvKJy4JXtUqaUu4=";
    })
  ];
    ...
    + ''
      substituteInPlace modules/ts/include/opencv2/ts/ts_gtest.h \
        --replace-fail \
          ...
          "#if defined(__GNUC__)"
    '' + ''
      # fetchpatch/patch(1) do not execute git rename directives (nixpkgs#32084),
      # so the file moves in the #29425 backport can end up half-applied.
      # Complete them explicitly:
      [ -f modules/dnn/src/glog_emulator.hpp ] || \
        mv modules/dnn/src/caffe/glog_emulator.hpp modules/dnn/src/glog_emulator.hpp
      [ -f modules/dnn/src/protobuf_io.hpp ] || \
        mv modules/dnn/src/caffe/caffe_io.hpp modules/dnn/src/protobuf_io.hpp
    '';
  ...
  propagatedBuildInputs = optionals enablePython [ python3Packages.numpy ];
  ...
  ++ optionals enablePython [
    (cmakeOptionType "path" "OPENCV_PYTHON_INSTALL_PATH" python3Packages.python.sitePackages)
  ]
  ...
  # By default $out/lib/pkgconfig/opencv5.pc looks something like this:
  #
  ...
  #   includedir_old=${prefix}//nix/store/g0wnfyjjh4rikkvp22cpkh41naa43i4i-opencv-4.0.0/include/opencv5/opencv
  #   includedir_new=${prefix}//nix/store/g0wnfyjjh4rikkvp22cpkh41naa43i4i-opencv-4.0.0/include/opencv5
  #   ...
  ...
  postInstall = ''
    ...
    mkdir "$cxxdev"
  ''
  # fix deps not propagating from opencv5.cxxdev if cuda is disabled
  ...
  + optionalString enableCuda ''
    substituteInPlace "$out/lib/cmake/opencv5/OpenCVConfig.cmake" \
      --replace-fail \
  ...
  #     inherit qimgv;
  #     withIpp = opencv5.override { enableIpp = true; };
  #   }
  #   // optionalAttrs (!enablePython) { pythonEnabled = python3Packages.opencv5; }
  #   // optionalAttrs (effectiveStdenv.buildPlatform != "x86_64-darwin") {
  #     opencv5-tests = callPackage ./tests.nix {
  #       inherit
  #         enableGStreamer
  #         enableGtk3
  #         runAccuracyTests
  ...
  #     no-libstdcxx-errors = callPackage ./libstdcxx-test.nix { attrName = "opencv5"; };
  #   };
  # }
